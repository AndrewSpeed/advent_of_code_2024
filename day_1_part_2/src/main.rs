...
fn calculate_similarity_score(left_list: Vec<i64>, right_list: Vec<i64>) -> Result<u64> {
    let similarity_score = left_list.iter().fold(0, |acc, left_elem| {
        let num_occurrences = right_list
            .clone()
            .into_iter()
            .filter(|right_elem| left_elem == right_elem)
            .count() as i64;
        let similarity = num_occurrences * left_elem;

        acc + similarity
    });

    Ok(similarity_score.try_into().unwrap())
}

fn similarity_score(input: &str) -> Result<u64> {
    let (left_list, right_list) = lines_to_lists(input);
    calculate_similarity_score(left_list, right_list)
}
...
fn main() -> Result<()> {
    ...
    let similarity = similarity_score(&contents)?;
...
3   3";
        ...
        assert_eq!(similarity_score(input).unwrap(), 31);
    }
}
